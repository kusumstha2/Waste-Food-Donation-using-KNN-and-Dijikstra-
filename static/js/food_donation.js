...
function viewDetails(index) {
  ...
  // Add the back button and book now button
  detailsDiv.innerHTML = `
    <div style="display: flex; gap: 10px; margin-bottom: 15px;">
      <button class="back-btn" onclick="goBack(${index})">← Back</button>
      <button class="book-now-btn" onclick="bookNow(${index})">Book Now</button>
    </div>
    <p><strong>Donor:</strong> ${donation.donorName}</p>
...
//   const donation = donations[index];
//   alert(You have booked the food donation: ${donation.foodName}. Please contact the donor at ${donation.location}.);
// }
...
function editDonation(index) {
  window.location.href = '/editdonation/';
}
